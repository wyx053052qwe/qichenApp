...
class CompanyController extends BackendController{
    ...
    public function add_gongzi(){
        if(IS_POST){
            if(count($_POST['cn'])!=count($_POST['en'])){
                IS_AJAX && $this->ajaxReturn(0,'中文英文不一致', '', 'add');
            }
            $config_arr = [];
            foreach ($_POST['cn'] as $key => $value) {
                if($value == ''){
                    IS_AJAX && $this->ajaxReturn(0,'请输入中文名', '', 'add');
                }
                if($_POST['en'][$key] == ''){
                    IS_AJAX && $this->ajaxReturn(0,'请输入英文名', '', 'add');
                }
                $config_arr[$key]['cn'] = $value;
                $config_arr[$key]['en'] = $_POST['en'][$key];
            }

            if(empty($_POST['id'])){
                $g = M('company_gongzi');
                $data['com_id'] = $_POST['com_id'];
                $data['config'] = json_encode($config_arr);
                $g->add($data);
            }else{
                $g = M('company_gongzi');
                $data['id'] = $_POST['id'];
            ...
            IS_AJAX && $this->ajaxReturn(1,'保存成功', '', 'add');
        }else{
            $id = I("get.id");
            $g = M('company_gongzi');
            $data = $g->where(['com_id'=>$id])->find();
            if(!empty($data)){
