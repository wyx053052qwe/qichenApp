        ...
        if ($key_type && $key) {
            switch ($key_type) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    $where['intention_jobs'] = arrray('like','%'.$key.'%');
            }
        } else {
            ...
            if($intention_jobs = I('request.intention_jobs',0,'intval')) {
//                var_dump($intention_jobs_id);die;
                $where['intention_jobs'] = array('gt', strtotime("-" . $intention_jobs . " day"));
            }
    ...
    public function jineng_cate(){
        $model = M('jineng_cate');
        if ($settr = I('request.settr', 0, 'intval')) {
            $where['add_time'] = array('gt', strtotime("-" . $settr . " day"));
        }
        $count      = $model->where($where)->count();// 查询满足要求的总记录数
        $pager = pager($count, 10);//实例化thinkphp内置的分页显示类
        $show       = $pager->fshow(true);// 分页显示输出
        $list = $model->where($where)->order('add_time DESC')->limit($pager->firstRow.','.$pager->listRows)->select();
        $this->assign('list',$list);// 赋值数据集
        $this->assign('page',$show);// 赋值分页输出
        $this->display('jineng_cate');
    }

    public function add_jineng_cate(){
        if(IS_POST){
            if(empty($_POST['name'])){
                $this->error('请输入分类名称');
            }
            $_POST['add_time'] = time();
            M('jineng_cate')->add($_POST);
    ...
    public function edit_jineng_cate(){
        if(IS_POST){
            if(empty($_POST['name'])){
                $this->error('请输入分类名称');
            }
            $_POST['add_time'] = time();
            M('jineng_cate')->save($_POST);
            $this->ajaxReturn(1,'成功');
        }else{
            $id = I('get.id');
            $data = M('jineng_cate')->where(['id'=>$id])->find();
            $this->assign('data',$data);
            $this->display();
        }
    }

    public function delete_jineng_cate(){
        $id = I('get.id');
        M('jineng_cate')->where(['id'=>$id])->delete();
        $this->success('删除成功');
    }

    public function jineng(){
        $model = M('jineng');
        $join = "left join qs_jineng_cate as a ON a.id = qs_jineng.cate_id";
        ...
        $list = $model->join($join)->where($where)->order('add_time DESC')->limit($pager->firstRow.','.$pager->listRows)->field("a.name as cate_name,qs_jineng.*")->select();
        //dump($list);die;
        $this->assign('list',$list);// 赋值数据集
        $this->assign('page',$show);// 赋值分页输出
    ...
    public function add_jineng(){
        if(IS_POST){
            if(empty($_POST['small_img'])){
                $this->ajaxReturn(0,'请上传图片');
            }
            if(empty($_POST['name'])){
                $this->ajaxReturn(0,'请填写培训名称');
            }

            if(empty($_POST['type'])){
                $this->ajaxReturn(0,'请填写培训类型');
            }

            if(empty($_POST['jigou'])){
                $this->ajaxReturn(0,'请填写培训机构');
            }
            if(empty($_POST['duixiang'])){
                $this->ajaxReturn(0,'请填写培训对象');
            }
            if(empty($_POST['feiyong'])){
                $this->ajaxReturn(0,'请填写培训费用');
            }
            if(empty($_POST['content'])){
                $this->ajaxReturn(0,'请填写培训内容');
            }
            $_POST['add_time'] = time();
            M('jineng')->add($_POST);
            $this->ajaxReturn(1,'成功');
    ...
    public function edit_jineng(){
        if(IS_POST){
            ...
            $id = I('get.id');
            $data = M('jineng')->where(['id'=>$id])->find();

            $this->assign('data',$data);
            $category = M('jineng_cate')->order('add_time DESC')->select();
            $this->assign('category',$category);
            $this->display('edit_jineng');
        }
    }

    public function delete_jineng(){
        $id = I('get.id');
        M('jineng')->where(['id'=>$id])->delete();
        $this->success('删除成功');
    }

    public function zaizhi(){
        $User = M('members_zaizhi'); // 实例化User对象
        $is_bind = I('request.is_bind', '', 'trim');
        $where = [];
        if($is_bind != ''){
            $where['is_bind'] = $is_bind;
        }
        if ($settr = I('request.settr', 0, 'intval')) {
            $where['dengji_time'] = array('gt', strtotime("-" . $settr . " day"));
        }
        ...
        $list = $User->join($join)->join($join1)->where($where)->order('dengji_time DESC')->limit($pager->firstRow.','.$pager->listRows)->field("qs_members_zaizhi.*,b.username as uid_name,c.companyname")->select();
        foreach ($list as $key => &$value) {
            $c = $value['ht_time'] - time();
            if($value['is_send'] == 1){
                if($c<7776000){
                    $value['is_f'] = 1;
                }else{
    ...
    public function send_zaizhi(){
        $uid = I('get.id',0,'intval');
        if(empty($uid)){
            $this->error('请选择用户！');
        }
        $zaizhi = M('members_zaizhi')->join("left join qs_company_profile as a ON a.id = qs_members_zaizhi.com_id")->where(['qs_members_zaizhi.uid'=>$uid,'qs_members_zaizhi.is_bind'=>1])->field("qs_members_zaizhi.*,a.companyname")->find();
        
        $content = "启辰人才提醒您，您的在职企业：".$zaizhi['companyname'].",合同于".date("Y-m-d",$zaizhi['ht_time'])."到期。";

        //dump($content);die;
        $where['uid'] = $uid;
        $where['type'] = 'weixin';
        $user = D('MembersBind')->where($where)->find();
        if(!$user || !$user['openid']) $this->error('该用户还没有绑定微信公众号！');
        
        $openid = $user['openid'];
        $reg = \Common\qscmslib\weixin::send_msg($openid,$content);
        if($reg === true){
            $this->success('发送成功！');
        }else{
            $this->error($reg);
    ...
    public function yijian_f(){
        $list = M('members_zaizhi')->join("left join qs_company_profile as a ON a.id = qs_members_zaizhi.com_id")->where(['qs_members_zaizhi.is_bind'=>1])->field("qs_members_zaizhi.*,a.companyname")->select();
        $new_arr = [];
        //dump($list);die;
        foreach ($list as $key => &$value) {
        ...
        foreach ($new_arr as $key => $value) {
            if(empty($value['uid'])){
                $this->error('在职人员'.$value['name']."未绑定");
            }
            $where['uid'] = $value['uid'];
            $where['type'] = 'weixin';
            $user = D('MembersBind')->where($where)->find();
            if(!$user || !$user['openid']) $this->error('在职人员'.$value['name']."未绑定微信公众号");
            $send_arr[$key]['openid'] = $user['openid']; 
            $send_arr[$key]['content'] = "启辰人才提醒您，您的在职企业：".$value['companyname'].",合同于".date("Y-m-d",$value['ht_time'])."到期。"; 
        }
        if(empty($send_arr)){
            $this->error('没有要发送的人员');
        }
        foreach ($send_arr as $key => $value) {
            $reg = \Common\qscmslib\weixin::send_msg($value['openid'],$value['content']);
            if(!$reg === true){
                $this->error($reg);
            }
        }
    ...
    public function gongzi_add(){
        if(IS_POST){
            if(empty($_POST['gongzi_time'])){
                $this->ajaxReturn(0,'请输入工资日期', '', 'gongzi');
            }

            if(empty($_POST['yingfa'])){
                $this->ajaxReturn(0,'请输入应发工资', '', 'gongzi');
            }

            if(empty($_POST['shifa'])){
                $this->ajaxReturn(0,'请输入实发工资', '', 'gongzi');
            }
            $data['gongzi_time'] = $_POST['gongzi_time'];
            $data['zaizhi_id'] = $_POST['zaizhi_id'];
            $data['com_id'] = $_POST['com_id'];
            $data['add_time'] = time();
            $data['yingfa'] = $_POST['yingfa'];
            $data['shifa'] = $_POST['shifa'];
            unset($_POST['zaizhi_id']);
            unset($_POST['yingfa']);
            unset($_POST['shifa']);
            unset($_POST['gongzi_time']);
            unset($_POST['com_id']);
            $data_arr = [];
            $k = 0;
            ...
            foreach ($_POST as $key => $value) {
                $data_arr[$k]['key'] = $key;
                $data_arr[$k]['value'] = $value;
                $k++;
            }
            
            $data['data'] = json_encode($data_arr);

            $is = M('members_gongzi')->add($data);
            if($is){
                $this->ajaxReturn(1,'添加成功', '', 'add');
            }else{
                $this->ajaxReturn(0,'服务器失败', '', 'add');
            }
        }else{
            $id = I('get.id');
            $data = M('members_zaizhi')->where(['id'=>$id])->find();
            $zaizhi_data = M("company_gongzi")->where(['com_id'=>$data['com_id']])->find();
            $company_gongzi = $this->get_gongzi_config($zaizhi_data['com_id']);
            $config = $company_gongzi['config'];
           
            $this->assign('zaizhi_id',$id);
            $this->assign('com_id',$zaizhi_data['com_id']);
            $this->assign('config',$config);
            $this->display('gongzi_add');
        }
    }

    public function gongzi_edit(){
        if(IS_POST){
            if(empty($_POST['gongzi_time'])){
                $this->ajaxReturn(0,'请输入工资日期', '', 'gongzi');
            }
            ...
            $data['data'] = json_encode($data_arr);
//            var_dump($data);die;
            $is = M('members_gongzi')->save($data);
            if($is){
                $this->ajaxReturn(1,'保存成功', '', 'gongzi');
            }else{
            ...
            $company_gongzi = $this->get_gongzi_config($data['com_id']);
            $config = $company_gongzi['config'];
            foreach ($data['data'] as $key => $value) {
                if($value['key'] == $config[$key]['en']){
            ...
            $this->display('gongzi_edit');
        }
    }

    public function gongzi_del(){
        $id = I('get.id');
        M('members_gongzi')->where(['id'=>$id])->delete();
        $this->success('删除成功',U('zaizhi'));
    }

    public function gongzi(){
        $zaizhi_id = I('get.zaizhi_id');
        if (!$zaizhi_id) {
            $this->ajaxReturn(0, '参数错误！');
        }
        $zaizhi_model = M('members_zaizhi');
        $zaizhi_data = $zaizhi_model->where(['id'=>$zaizhi_id])->find();
        if(empty($zaizhi_data['uid'])){
            $this->ajaxReturn(0, '未绑定用户');
        }
        $userinfo = D('Members')->get_user_one(array('uid' => $zaizhi_data['uid']));
        $com_arr = [];
        if(!empty($zaizhi_data['com_id'])){
            array_push($com_arr,$zaizhi_data['com_id']);
        }
        if(!empty($zaizhi_data['agein_com_id'])){
            array_push($com_arr,$zaizhi_data['agein_com_id']);
        }
        //dump($com_arr);die;
        $com_list = M('company_profile')->where(array('id' => array('in', $com_arr)))->field('id,companyname')->select();
        $gongzi_model = M('members_gongzi');
        
        $return_config = [];
        foreach ($com_list as $key => $value) {
            $config = $this->get_gongzi_config($value['id']);
            $config['companyname'] = $value['companyname'];
            $return_config[$key] = $config;    
        }
        $this->assign('zaizhi_id',$zaizhi_id);
        $this->assign('userinfo',$userinfo);
        $this->assign('return_config',$return_config);
//        dump($zaizhi_id);die;
        $html = $this->fetch('gongzi');
        $this->ajaxReturn(1, '获取数据成功！', $html);

    }

    public function gongzi_log(){
        $com_id = I('get.com_id');
        $zaizhi_id = I('get.zaizhi_id');
        $config = $this->get_gongzi_config($com_id);
        $gongzi_model = M('members_gongzi');
        $data = $gongzi_model->where(['zaizhi_id'=>$zaizhi_id,'com_id'=>$com_id])->select();
        ...
        $this->assign('data',$data);
//        dump($data);die;
        $html = $this->fetch('gongzi_log');
        $this->ajaxReturn(1, '获取数据成功！', $html);
    }

    public function zaizhi_add(){
        if(IS_POST){
            if(empty($_POST['name'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入姓名', '', 'add');
                $this->error('请输入姓名');
            }
            if(empty($_POST['id_card'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入身份证号码', '', 'add');
                $this->error('请输入身份证号码');
            }

            if(empty($_POST['gongzi_card'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入工资卡号', '', 'add');
                $this->error('请输入工资卡号');
            }

            if(empty($_POST['id_card'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入联系方式', '', 'add');
                $this->error('请输入联系方式');
            }

            if(empty($_POST['ht_time'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入合同到期时间', '', 'add');
                $this->error('请输入合同到期时间');
            }

            if(empty($_POST['com_id'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入正确的企业名称', '', 'add');
                $this->error('请输入正确的企业名称');
            }
            $_POST['ht_time'] = strtotime($_POST['ht_time']);
            if(empty($_POST['ht_time'])){
                IS_AJAX && $this->ajaxReturn(0,'时间格式输入错误', '', 'add');
                $this->error('时间格式输入错误');
            }
            $zaizhi = M('members_zaizhi'); 
            $zaizhi->add($_POST);
            IS_AJAX && $this->ajaxReturn(1,'添加成功', '', 'add');
            $this->success('添加成功');
        }
        $this->display('zaizhi_add');
    }

    public function zaizhi_delete(){
        $tuid = I('post.tuid', '', 'trim');
        !$tuid && $this->error('你没有选择在职人员！');
        $v = '';
        foreach ($tuid as $key => $value) {
            $v = $v.$value . ',' ;
        }
        $v = substr($v,0,strlen($str)-1);
        $zaizhi = M('members_zaizhi'); 
        $zaizhi->delete($v);
         $this->success('删除成功！');
    }

    public function zaizhi_edit(){
        if(IS_POST){
            if(empty($_POST['name'])){
                IS_AJAX && $this->ajaxReturn(0,'请输入姓名', '', 'add');
                $this->error('请输入姓名');
            ...
            $_POST['dengji_time'] = time();
            $_POST['lizhi_time'] = 0;
            $_POST['is_bind'] = 1;
            $zaizhi = M('members_zaizhi');
            $zaizhi->save($_POST);
            IS_AJAX && $this->ajaxReturn(1,'修改成功', '', 'add');
            ...
            $join1 = "left join qs_company_profile as c ON c.id=qs_members_zaizhi.com_id";
            $zaizhi = M('members_zaizhi');
            $data = $zaizhi->join($join1)->where(['qs_members_zaizhi.id'=>$id])->field("qs_members_zaizhi.*,c.companyname")->find();
            $this->assign('data',$data);
            //dump($data);die;
            $this->display('zaizhi_edit');
        }
    ...
    public function lizhi(){
        $id = I('get.id');
        $zaizhi = M('members_zaizhi')->where(['id'=>$id])->find();
        $zaizhi['agin_com_id'] = $zaizhi['com_id'];
        $zaizhi['com_id'] = 0;
        $zaizhi['dengji_time'] = 0;
        $zaizhi['is_bind'] = 0;
        $zaizhi['lizhi_time'] = time();
        M('members_zaizhi')->save($zaizhi);
        IS_AJAX && $this->ajaxReturn(1,'离职成功', '', 'add');
        $this->success('离职成功');
    }

    public function lizhi_list(){
        $where = [];
        if ($settr = I('request.settr', 0, 'intval')) {

            $where['qs_members_zaizhi_lizhi.add_time'] = array('gt', strtotime("-" . $settr . " day"));
        }
        $join = "left join qs_members_zaizhi as a ON a.id=qs_members_zaizhi_lizhi.zaizhi_id";
        $shenqing = M('members_zaizhi_lizhi');
        $count      = $shenqing->where($where)->count();// 查询满足要求的总记录数
        $pager = pager($count, 10);//实例化thinkphp内置的分页显示类
        $show       = $pager->fshow(true);// 分页显示输出
        $list = $shenqing->join($join)->order('add_time DESC')->field('qs_members_zaizhi_lizhi.*,a.name,a.is_bind')->select();
        foreach ($list as $key => &$value) {
        ...
        $this->display('lizhi_list');
    }

    public function lizhi_del(){
        $id = I('get.id');
        M('members_zaizhi_lizhi')->where(['id'=>$id])->delete();
        $this->success('删除ok');
    }
    ...
    public function zaizhi_shenqing(){
        $where = [];
        if ($settr = I('request.settr', 0, 'intval')) {

            $where['qs_members_zaizhi_shenqing.add_time'] = array('gt', strtotime("-" . $settr . " day"));
        }
        $where['qs_members_zaizhi_shenqing.status'] = 0;
        //dump($where);die;
        $join = "left join qs_members as b ON b.uid=qs_members_zaizhi_shenqing.uid";
        $join1 = "left join qs_company_profile as c ON c.id=qs_members_zaizhi_shenqing.com_id";
        $shenqing = M('members_zaizhi_shenqing');
        $count      = $shenqing->where($where)->count();// 查询满足要求的总记录数
        $pager = pager($count, 10);//实例化thinkphp内置的分页显示类
        $show       = $pager->fshow(true);// 分页显示输出
        $list = $shenqing->join($join)->join($join1)->order('add_time DESC')->where($where)->limit($pager->firstRow.','.$pager->listRows)->field("qs_members_zaizhi_shenqing.*,b.username as uid_name,c.companyname")->select();
        //dump($list);die;
        $this->assign('list',$list);
        $this->assign('page',$show);
        $this->display('zaizhi_shenqing');
    }

    public function shenqing_true(){
        $id = I('get.id');
        $shenqing = M('members_zaizhi_shenqing')->where(['id'=>$id])->find();
        $zaizhi = M('members_zaizhi');
        $zaizhi_data = $zaizhi->where(['uid'=>$shenqing['uid']])->find();
        if(!empty($zaizhi_data)){
            if($zaizhi_data['id_card'] != $shenqing['id_card'] && $zaizhi_data['name'] != $shenqing['name']){
                $this->error('身份证信息不符');
            }
        }
        $zaizhi_is = $zaizhi->where(['id_card'=>$shenqing['id_card'],'name'=>$shenqing['name']])->find();
        //dump($zaizhi_is);die;
        if(empty($zaizhi_is)){
            $data['uid'] = $shenqing['uid'];
            $data['com_id'] = $shenqing['com_id'];
            $data['id_card'] = $shenqing['id_card'];
            $data['name'] = $shenqing['name'];
            $data['dengji_time'] = time();
            $data['is_bind'] = 1;
            $is = $zaizhi->add($data);
            if($is){
                $this->success('成功',U('zaizhi'));
            }else{
                $this->error('服务器失败');
            }
        }else{
            $zaizhi_is['uid'] = $shenqing['uid'];
            $zaizhi_is['com_id'] = $shenqing['com_id'];
            $zaizhi_is['id_card'] = $shenqing['id_card'];
            $zaizhi_is['name'] = $shenqing['name'];
            $zaizhi_is['lizhi_time'] = 0;
            $zaizhi_is['is_bind'] = 1;
            $zaizhi_is['dengji_time'] = time();
            if($zaizhi_is['agin_com_id'] == $zaizhi_is['com_id']){
                $zaizhi_is['com_id'] = 0;
            }
            $is = $zaizhi->save($zaizhi_is);
            if($is){
                M('members_zaizhi_shenqing')->where(['id'=>$id])->delete();
                $this->success('成功',U('zaizhi'));
            }else{
                $this->error('服务器失败');
            }
    ...
    public function shenqing_false(){
        if(IS_POST){
            
            if(empty($_POST['yuanyin'])){
                $this->ajaxReturn(0, '请填写原因','');
            }
            $_POST['status'] = 2;
            M('members_zaizhi_shenqing')->save($_POST);
            $this->ajaxReturn(1, '发送成功！','');
        }else{
            $id = I('get.id');
            $this->assign('id',$id);
            $html = $this->fetch('shenqing_false');
            $this->ajaxReturn(1, '获取数据成功！', $html);
        }
    }

    public function zaizhi_zhengming(){
        $where = [];
        $join = "left join qs_members as b ON b.uid=qs_members_zaizhi_zhengming.uid";
        if ($settr = I('request.settr', 0, 'intval')) {

            $where['qs_members_zaizhi_zhengming.add_time'] = array('gt', strtotime("-" . $settr . " day"));
        }
        $status = I('request.status');
       
        if ($status != '') {
            
            $where['qs_members_zaizhi_zhengming.status'] = $status;
        }
       
        $zhengming = M('members_zaizhi_zhengming');
        $count      = $zhengming->where($where)->count();// 查询满足要求的总记录数
        $pager = pager($count, 10);//实例化thinkphp内置的分页显示类
        $show       = $pager->fshow(true);// 分页显示输出
        $list = $zhengming->join($join)->where($where)->order('add_time DESC')->limit($pager->firstRow.','.$pager->listRows)->field("qs_members_zaizhi_zhengming.*,b.username as uid_name")->select();
        foreach ($list as $key => &$value) {
            $value['img'] = json_decode($value['img'],true);
            $value['files'] = json_decode($value['files'],true);
        };
//        dump($list);die;
        $this->assign('list',$list);
        $this->assign('page',$show);
        $this->display('zaizhi_zhengming');
    }
    

    public function zhengming_kaiju(){
        $id = I('get.id');
        M('members_zaizhi_zhengming')->where(['id'=>$id])->save(['status'=>1]);
        $this->success('开具成功');
    }

    public function zhengming_del(){
        $id = I('get.id');
        M('members_zaizhi_zhengming')->where(['id'=>$id])->delete();
        $this->success('删除ok');
    }
    ...
    public function huodong(){
        $where = [];
        if ($settr = I('request.settr', 0, 'intval')) {
            $where['qs_members_zaizhi_huodong.add_time'] = array('gt', strtotime("-" . $settr . " day"));
        }
        $key_type = I('request.key_type', 0, 'intval');
        $key = I('request.key', '', 'trim');
        if ($key_type && $key) {
        ...
        $count      = $huodong->where($where)->count();// 查询满足要求的总记录数
        $pager = pager($count, 10);//实例化thinkphp内置的分页显示类
        $show       = $pager->fshow(true);// 分页显示输出
        $list = $huodong->where($where)->limit($pager->firstRow.','.$pager->listRows)->select();
        //dump($list);die;
        $this->assign('list',$list);
        $this->assign('page',$show);
    ...
    public function huodong_add(){
        if(IS_POST){
            if(empty($_POST['name'])){
                $this->ajaxReturn(0,'请输入活动标题');
            }

            if(empty($_POST['content'])){
                $this->ajaxReturn(0,'请输入内容');
            }
            if(empty($_POST['small_img'])){
                $this->ajaxReturn(0,'请上传活动缩略图');
            }
            if(empty($_POST['start_time'])){
                $this->ajaxReturn(0,'请输入活动开始时间');
            }
            if(empty($_POST['end_time'])){
                $this->ajaxReturn(0,'请输入活动结束时间');
            }
            if(empty($_POST['descript'])){
                $this->ajaxReturn(0,'请输入活动描述');
            }
            $data['name'] = $_POST['name'];
            $data['add_time'] =time();
            $data['img'] = $_POST['small_img'];
            ...
            if(!empty($start) && !empty($end)){
                if($end<$start){
                    $this->ajaxReturn(0,'结束时间不能大于开始时间');
                }

            }else{
                $this->ajaxReturn(0,'时间格式输入错误');
            }
            $data['start_time'] = $start;
            $data['end_time'] = $end;
            //dump($data);die;
            M('members_zaizhi_huodong')->add($data);
    ...
    public function huodong_edit(){
        if(IS_POST){
            ...
            M('members_zaizhi_huodong')->save($data);
    ...
    public function huodong_del(){
        ...
        M('members_zaizhi_huodong')->where(['id'=>$id])->delete();
        $this->success('删除成功！');
    }
    ...
    public function huodong_baoming(){
        ...
        $join = "left join qs_members as b ON b.uid=qs_members_zaizhi_huodong_baoming.uid";
        $join1 = "left join qs_members_zaizhi as a ON a.id=qs_members_zaizhi_huodong_baoming.zaizhi_id";
        $join2 = "left join qs_members_zaizhi_huodong as c ON c.id=qs_members_zaizhi_huodong_baoming.huodong_id";
        $count      = $baoming->where($where)->count();// 查询满足要求的总记录数
        $pager = pager($count, 10);//实例化thinkphp内置的分页显示类
        $show       = $pager->fshow(true);// 分页显示输出
        $list = $baoming->join($join)->join($join1)->join($join2)->where($where)->limit($pager->firstRow.','.$pager->listRows)->field("qs_members_zaizhi_huodong_baoming.*,a.name,b.username as uid_name,c.name as huodong_name")->select();
        //dump($list);die;
        $this->assign('count',$count);
        $this->assign('list',$list);
        $this->assign('page',$show);
        $this->display('huodong_baoming');
    }

    public function son_list(){
        $uid = I('get.uid');
        $list = M('members')->where(['p_id'=>$uid])->select();
        $arr = [];
        foreach ($list as $key => $value) {
            $zaizhi = M('members_zaizhi')->where(['uid'=>$value['uid'],'is_bid'=>1])->field('id,dengji_time,name,uid,id_card,com_id')->find();
            if(!empty($zaizhi)){
                $com = M('company_profile')->where(['id'=>$zaizhi['com_id']])->field('companyname')->find();
                if(empty($com)){
                    $zaizhi['companyname'] = "数据错误";
                }else{
                    $zaizhi['companyname'] = $com['companyname'];
                }
                
                array_push($arr,$zaizhi);
            }

        }
        $this->assign('list',$arr);
        $html = $this->fetch('son_list');
        $this->ajaxReturn(1,'',$html);
    }

    public function jifen(){
        $uid = I('get.uid');
        $members = M('members')->where(['uid'=>$uid])->find();
        $this->assign('members',$members);
        $html = $this->fetch('jifen');
        $this->ajaxReturn(1,'',$html);
    }
    public function jifen_send(){
        if(IS_POST){
            
            if(empty($_POST['uid'])){
                $this->ajaxReturn(0,'id用户错误','','');
            }
            $uid = intval($_POST['uid']);
            if(empty($_POST['jifen'])){
                $this->ajaxReturn(0,'请输入积分数量','','');
            }

            $jifen_num = intval($_POST['jifen']);
            if(empty($_POST['content'])){
                $this->ajaxReturn(0,'请输入积分备注','','');
            }
            //dump($uid);die;
            $jifen = D("MembersPoints");
            $jifen->report_deal($uid,1,$jifen_num);
            $cn = $_POST['content'];
            $is = M('members_handsel')->add(['uid'=>$uid,'htype'=>'zengsong','htype_cn'=>$cn,'operate'=>1,'points'=>$jifen_num,'addtime'=>time()]);
            if($is){
                $this->ajaxReturn(1,'赠送成功','','');
            }else{
                $this->ajaxReturn(0,'赠送失败','','');
            }
        }
    }
    /**
