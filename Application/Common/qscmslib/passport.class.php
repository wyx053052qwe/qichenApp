    ...
    private function _local_add($add_data) { 
        ...
        if (false !== $user_mod->create($add_data)) {
            ...
            if (!$uid = $user_mod->add()) {
                ...
                $add_data['uid'] = $uid;
                $u = $user_mod->get_user_one(['uid'=>$uid]);
                $f_mem = $user_mod->where(['uid'=>$u['p_id']])->find();
                if(!empty($f_mem)){
                    $jifen = D("MembersPoints");
                    $jifen->report_deal($f_mem['uid'],1,200);
                    $cn = "推荐用户id为：".$f_mem['uid']."用户";
                    M('members_handsel')->add(['uid'=>$f_mem['uid'],'htype'=>'tuijian','htype_cn'=>$cn,'operate'=>1,'points'=>200,'addtime'=>time()]);
                }
                return $add_data;
            }
        }
