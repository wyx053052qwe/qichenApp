...
function contact_hide($data, $IsWhat = 2) {
    if ($IsWhat == 1) {
        ...
        return preg_replace('/(1[345789]{1}[0-9])[0-9]{4}([0-9]{4})/i', '$1****$2', $data);
    } elseif ($IsWhat == 3) {
...
function GetIpLookup($ip = '') {
    ...
    $res = @file_get_contents('https://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js&ip=' . $ip);
    if (empty($res)) {
        return false;
    }
    $jsonMatches = array();
...
function resume_refreshtime_day($refreshtime){
    $day = intval(($refreshtime - time()) / 86400);
    if($day < 1){
        $val = 1;
    }elseif($day < 3){
        $val = 3;
