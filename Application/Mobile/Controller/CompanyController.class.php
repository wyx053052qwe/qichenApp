...
class CompanyController extends MobileController {
    ...
    public function tuijian(){
        ...
        $filename = 'data/upload/qrcode/id'.$members['uid'].'code.PNG';
        $url = "https://www.qichenhr.xyz".U("members/apilogin_binding",array('p_id'=>$members['uid']));
        if(!file_exists($filename)){
            $this->qs_qrcode_png($url,$filename);
        }
        $this->wx_share();
    ...
    private function qs_qrcode_png($url,$filename=false){
        ...
        $logo = 'data/upload/resource/64.jpg';
        $QR = $filename; 
        if(file_exists($logo)){
    ...
    public function index() {
        ...
        $this->assign('favorites_num',$this->get_favorites_num());
    ...
    public function resume_favorites() {
        ...
        $config = D('Config')->config_cache();
        if($config['qscms_m_template_dir'] == 'qichen'){
            $this->display('resume_favorites1');
        }else{
            $this->display();
        }
    }

    private function get_favorites_num(){
        $where['company_uid'] = C('visitor.uid');
        $favorites = D('CompanyFavorites')->get_favorites($where);
        return count($favorites['list']);
    }
