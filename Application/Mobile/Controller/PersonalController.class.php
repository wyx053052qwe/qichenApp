...
class PersonalController extends MobileController {
    ...
    */
    public function index() {
        ...
        $zaizhi = M('members_zaizhi')->where(['uid'=>$uid,'is_bind'=>1])->find();
        $this->assign('zaizhi',$zaizhi);
    ...
    */
    public function resume_edit_basis() {
        ...
        if (IS_AJAX) {
            ...
            $data['url'] = I('request.news', 0, 'intval') ? U('personal/resume_replenish', array('pid' => $resume['id'], 'news' => 1)) : U('personal/resume_edit1', array('pid' => $resume['id']));
    ...
    */
    ...
    public function jobs_interview1() {
        $this->check_params();
        $where['resume_uid'] = C('visitor.uid');
        $look = I('get.look', 0, 'intval');
        $look && $where['personal_look'] = $look;
    ...
    public function jobs_favorites1() {
        $this->check_params();
        $where['personal_uid'] = C('visitor.uid');
        $settr = I('get.settr', 0, 'intval');
        $settr && $where['addtime'] = array('gt', strtotime("-" . $settr . " day")); //筛选 收藏时间
    ...
    public function attention_com1() {
        $this->check_params();
        $company = D('PersonalFocusCompany')->get_focus_company(array('uid' => C('visitor.uid')), 10, true);
        $this->assign('company', $company);
        $this->_config_seo(array('title' => '关注的企业', 'header_title' => '关注的企业'));
