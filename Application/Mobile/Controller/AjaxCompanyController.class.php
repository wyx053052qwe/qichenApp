...
class AjaxCompanyController extends MobileController{
    ...
    public function resume_down_confirm(){
        ...
        if(C('visitor.status')==2){
            $this->ajaxReturn(1,'您的账号处于暂停状态，请联系管理员设为正常后进行操作','status');
        }
        if(C('qscms_down_resume_limit') != 0){
            $company_profile = M('CompanyProfile')->where(array('uid' => C('visitor.uid')))->find();
            !$company_profile && $this->ajaxReturn(1,'为了达到更好的招聘效果，请先完善您的企业资料！','company');
            ...
            foreach ($company_profile as $key => $value) {
                if (in_array($key, $array) && empty($value)) {
                    $this->ajaxReturn(1,'为了达到更好的招聘效果，请先完善您的企业资料！','company');
                    break;
                }
            }
        }
        if(C('qscms_down_resume_limit')==3){
            $companyinfo = M('CompanyProfile')->where(array('uid'=>$user['uid']))->find();
            if ($companyinfo['audit']!=1)
            {
                $this->ajaxReturn(1,'你的营业执照未通过认证导致无法下载简历！','audit');
            }
        }else if(C('qscms_down_resume_limit')==1){
            $user_jobs=D('Jobs')->count_auditjobs_num(C('visitor.uid'));
            if ($user_jobs==0){
                $this->ajaxReturn(1,'你没有发布职位或审核未通过导致无法下载简历！','jobs');
            }
        }
        ...
        $resume_info = D('Resume')->where(array('id'=>array('eq',$rid)))->select();
        $resume_counts = resume_refreshtime_day($resume_info['refreshtime']);
        ...
        if ($my_setmeal['download_resume'] < $resume_counts)// 套餐内简历下载点数不足
        {
            if(C('qscms_resume_download_quick')==1){
				if($my_points>=C('qscms_download_resume_price')*C('qscms_payment_rate') && C('qscms_down_resume_by_points')==1){
					$tip = '<div class="dialog_notice">您的套餐内简历下载点数不足，下载该简历需要扣除 <span class="font_yellow">'.intval(C('qscms_payment_rate')*C('qscms_download_resume_price')).'</span> '.C('qscms_points_byname').'，是否继续下载？<br /><div class="dialog_tip font10 font_gray9">您当前拥有 <span class="font_yellow">'.$my_points.'</span> '.C('qscms_points_byname').'</div></div>';
					$mode = 'points';
				}else{
				...
				$this->ajaxReturn(1,'您套餐中剩余的下载简历点数不足，请升级套餐后继续下载','setmeal');
			}
        }else{
            $tip = '<div class="dialog_notice">下载这份简历会扣除 <span>'.$resume_counts.'</span> 点简历点，是否继续下载？</div>';
            $mode = 'free';
        }
        $this->ajaxReturn(1,$tip,$mode);
    }
