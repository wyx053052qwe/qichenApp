...
$(function() {
    ...
    //地区筛选
    $('#j-cate-city').die().live('click', function () {
        cateCitySome();
    })
    if (eval($.fn.cookie('sRange'))) {
        $('#j-cate-range').removeClass('qs-btn-orange').addClass('qs-btn-border-orange');
        setRangeHtml();
    } else {
        $('#j-cate-city').removeClass('qs-btn-orange').addClass('qs-btn-border-orange');
    }
    // 附近筛选
    $('#j-cate-range').die().live('click', function () {
        setRangeHtml();
    })
    // 生产附近筛选条件
    function setRangeHtml() {
        var dataRange = new Array("0,距离不限","1,1公里","3,3公里","5,5公里","10,10公里");
        var dataHtml = '<div class="f-box-inner">';
        dataHtml += '<div class="ncc-list-g">';
        dataHtml += '<div class="ncc-list-c Js-ncc-list-c">';
        for (i = 0; i < dataRange.length; i++) {
            dataHtml += '<div class="c rangeC" data-code="'+ dataRange[i].split(',')[0] +'"><div class="c-icb"><div class="c-ic"></div></div><div class="c-txb">'+ dataRange[i].split(',')[1] +'</div><div class="clear"></div></div>';
        }
        dataHtml += '</div>';
        dataHtml += '</div>';
        dataHtml += '</div>';
        dataHtml += '<div class="f-btn-submit qs-center"><div class="qs-btn qs-btn-inline qs-btn-medium qs-btn-orange" id="j-cate-city" style="margin-right:.25rem;">按地区</div><div class="qs-btn qs-btn-inline qs-btn-medium qs-btn-border-orange" id="j-cate-range" style="margin-right:.25rem;">附近职位</div><div class="qs-btn qs-btn-inline qs-btn-medium qs-btn-orange" id="j-no-limit-city">不限</div></div>';
        $('.f-box-city').html(dataHtml);
        if ($('.qs-temp-code-range').val()) {
            $('.rangeC').each(function () {
                if ($(this).data('code') == $('.qs-temp-code-range').val()) {
                    $(this).addClass('chk');
                }
            })
        }
    }
    $('.rangeC').die().live('click', function () {
        $('.qs-temp-code-range').val($(this).data('code'));
        $('.qs-temp-code-city').val('');
        $('body').removeClass('filter-fixed');
        $('.f-box-city').addClass('qs-hidden');
        $('#f-mask').hide();
        $('.qs-temp').removeClass('active');
        $.fn.cookie('sRange',1);
        goPage();
