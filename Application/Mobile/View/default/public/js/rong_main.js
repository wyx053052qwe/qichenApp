        ...
        switch(message.messageType){
            case RongIMClient.MessageType.TextMessage:
                ...
                if(rongUser.sendUser && message.senderUserId==rongUser.sendUser.id){
                    ...
                    var recDate = new Date(a.addtime * 1000);
                    a.addtime = recDate.getHours() + ':' + recDate.getMinutes();
                    ...
                    if(rongConfig.messageList){
                        var t = String(message.sentTime).substr(0,10),
                            f = $('.imlist-img[uid="'+message.senderUserId+'"]');
                        var cuM = new Date(t * 1000).getMinutes();
                        if (cuM < 10) {
                            cuM = '0' + cuM;
                        }
                        var time = new Date(t * 1000).getHours() + ':' + cuM;
                        if(f[0]){
            ...
            onSuccess: function(list) {
                $.getJSON(rongConfig.messageUnreadUrl,function(r){
                    if(r.status == 1 && r.data){
                        $('#J_imStatus').append('<div class="m_dot"></div>');
                    }
                });
            },
