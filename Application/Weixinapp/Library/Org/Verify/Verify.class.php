...
class Verify {
     ...
     */
    public function check($code, $secode = '') {
        // 验证码不能为空
        if(empty($code) || empty($secode)) {
            return false;
        }
        if($this->jiami($code) == $secode){
            return true;
        }
    ...
    public function entry($id = '') {
        ...
        $res['img'] = "data:image/png;base64,". base64_encode ($image_data);
        $res['id'] = $se_code;
        return $res;
    ...
    private function jiami($code){
        $code = strtolower($code);
        return md5(md5($code).'ThinkPHP.CN');
    }
